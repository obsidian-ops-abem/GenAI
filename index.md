...
- [[14_Claude Code×Jev コンテキスト91%節約]] — @yama_threadsによるJevコンパクション利用の日本語ハウツー。「要約」(生成・1分・欠落リスク)と「選別」(判断・1秒・関連性フィルター)の違い・「後から掃除でなく最初から散らかさない」・導入3ルート(Playground1問/外部スクリプト/Hook自動化)・修正はモデルでなくstate・2層防御(ルータ+ゲート)・落とし穴(定額プランの節約は使用枠温存で月額不変・Jev側課金は別途)。1M→86K・1秒はAlex Volkov実測（本文はクリップ全文を使用・記事中に宣伝を含む）の要約
...
- [[05_Jev Engineering 決定と生成を分離するSystem One]] — @0xMovez紹介/Jev by TypeSafe AI。System One決定特化モデル(状態+定義済み質問→型付き回答+確率・生成なし・200倍速/400倍安・$0.042/MTok)。「テキストを作る操作はLLM・選択/スコア/yes-noはJev・正確なルールはコード」の3分離。3質問型(Choice/Score/Noul)・並行評価・ハーネス2層(モデルルーター+AutoModeゲート)・「ハーネスは同じモデルで78%と42%の差」・インスタント・コンパクション(1M→86Kを1秒・要約でなく関連性フィルタ)。「思考と決定を分離した少数が桁違いに安く速い」（本文はユーザー提供クリップから再構成・プロモーション色強いが設計知見は汎用）の要約
- [[06_Claude Codeのmodelの仕様 セッション別固定とs決定]] — @oikon48共有。通常の`/model`選択は「以後新規起動するセッションの既定モデル」を変える(既存セッションへの即時影響ではない・リプライで訂正確定)。個別セッションだけモデルを固定するには`/model`選択時に`s`で決定するか起動時`--model`オプションを使う。`claude --model fable[1m]`指定は逆に起動しない等バージョン依存の食い違いも報告あり（本文はXポスト+リプライスレッド全文を使用。02_wiki/summaries/06_claude/直下の未Ingest混入を是正・2026-09-23）の要約
- [[07_Jev Engineering実践ガイド 分割と失敗モード]] — Khairallah(@eng_khairallah1)によるJevビルドガイド。@0xMovez版(05_)の補完で失敗モードとスコアカードが差分。「Jevは賢いif文」・3バケット・RLCD較正(confidenceをアクション毎にコストでスケール・不可逆には根拠でない)・「全部一度に聞く(13問バッチ=12倍安10倍速)」・失敗モード6種(文字通り読む/計算不能/日付はテキスト/context rot/世界知識なし/stateは敵対扱いでない=インジェクション面)・スコアカード(67.8%・自己運営・「ハルシネーション不能」は構成による主張)。TypeSafeはDiogo Almeida(RLHF/InstructGPT共同発明者)が創業（本文はクリップ全文を使用）の要約
- [[08_Jevは誰のためのモデルか 汎用Decision Model]] — @hkunimitsuによるJevの批判的ポジショニング。「個人が今すぐ直接使う理由はほとんどない」・エージェント裏側の1日20億decisionを切り出す汎用Decision Model・LLM=「次に何を書くか」/Jev=「どの選択肢が正しいか」・較正(90%と言ったら実際約90%正しい)で3段運用(自動実行/別AI確認/人間)・Synthetic Data工場・将来は大手にも実装され一般ユーザーは無意識に恩恵。Jevノート群の冷却材（本文はクリップ全文を使用）の要約
...
- [[16_QWen Image Edit 軽量QLoRAでジブリ風画像生成]] — @ai_hakase_紹介。SDXLをQLoRA 4bitで軽量化しRTX 3070Tiクラスで約1分・4-6ステップでジブリ風アニメ画像生成。3種Ghibli LoRA(Old/New/Film)+Canny線画→潜空間+Grad-CAM自動補正+セグメンテーション背景認識でバッチサイズ1でも高速。※本ボルト主軸(エージェント/PKM)の周縁・画像生成AI技術解説（本文はユーザー提供クリップから再構成）の要約
- [[17_自作Jevを17ドルでファインチューニング Together Tev]] — @nutlope(Together AI)による自作Jevハウツー。together/Tev1-4B-experimental(Qwen3.5 4B・オープンウェイト)を8種37,840例(MultiNLI/BoolQ/Banking77/SST-5+policies/routing/taxonomy)で**$17**・数時間でファインチューニング・H100専用エンドポイントでデプロイ・JSON(state+question+options)でクエリ。TypeSafe Jev(クローズド)へのオープン替代・プライベートデータ訓練可・ロックインなし（本文はクリップ全文を使用）の要約
- [[18_Jev入門 System One Modelの日本語解説 npaka]] — npaka(note)によるJevの日本語入門決定版。「非構造化状態→型付き確率的判断」・高速性の源泉=parallel sampler(複数判断の並列出力)+RLCD・DOOMを10Hz・約7ドル/時・「型が正しい」と「判断が正しい」は別(ハルシネーションしないの真意)・10Hz上位判断は可能だがサーボ制御は不可・日本からはNW遅延に注意・「スマートなif文」。Jev系5ノートの読み順は本記事→05_→07_→08_→14_（本文はクリップ全文を使用）の要約
