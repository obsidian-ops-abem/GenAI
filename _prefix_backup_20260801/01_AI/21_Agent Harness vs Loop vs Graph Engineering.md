---
title: "21_Agent Harness vs Loop vs Graph Engineering"
...
- **Loop から始める**（基本）→ [[12_ループエンジニアリング14ステップ]]
- **Graph へ**: 非線形・分岐・並列、複数特化ステップ間の状態共有、単一ループの記憶過多/待ち状態 → [[19_Graph Engineering Clearly Explained]]
...
- [[06_LOOP vs GRAPH vs HARNESS ENGINEERING]] — 同3層の診断フレーム
- [[10_Prompt to Graph Engineering 5層の統一モデル]] — Prompt/Context/**Harness**/Loop/Graph の5層（前編）
- [[19_Graph Engineering Clearly Explained]] — Graph 層の実践解説（続編）
- [[07_LOOP→GRAPH→HARNESSパイプラインを一気通貫で実装する]] — 3層の実装チュートリアル
...
- 3層診断フレーム → [[06_LOOP vs GRAPH vs HARNESS ENGINEERING]]
- 5層統一モデル（前編） → [[10_Prompt to Graph Engineering 5層の統一モデル]]
- Graph 層の深掘り（続編） → [[19_Graph Engineering Clearly Explained]]
- 3層の実装 → [[07_LOOP→GRAPH→HARNESSパイプラインを一気通貫で実装する]]
- Loop 層のロードマップ → [[12_ループエンジニアリング14ステップ]]
