...
created: 2026-08-13
description: "where loop engineering sits in the stack, the four loop types in Claude Code, the evaluator that decides whether any of it works, and the fo..."
tags:
  - "clippings"
---
![画像](https://pbs.twimg.com/media/HOiD79EWwAEnPYM?format=jpg&name=large)
...
![画像](https://pbs.twimg.com/media/HOXQH4PXwAAXYLe?format=jpg&name=large)
...
![画像](https://pbs.twimg.com/media/HOXR9t5XMAAbw_H?format=png&name=large)
...
2. **Hand off** - give the task to an agent in its own isolated workspace
3. **Verify** - a second agent, different instructions, tries to break it
4. **Persist** - the outcome lands on disk, not in the chat
5. **Schedule** - the next run gets set, and today's leftovers carry over

Take any one of those out and the thing either stalls or spins in place while the meter runs

![画像](https://pbs.twimg.com/media/HOXTZI8XkAArPCu?format=jpg&name=large)
...
![画像](https://pbs.twimg.com/media/HOXT4mMX0AALl2B?format=jpg&name=large)
...
<video preload="none" tabindex="-1" playsinline="" aria-label="埋め込み動画" poster="https://pbs.twimg.com/amplify_video_thumb/2082305193253728256/img/OkI0GkeDkXDwZlOb.jpg" style="width: 100%; height: 100%; position: absolute; background-color: black; top: 0%; left: 0%; transform: rotate(0deg) scale(1.005);"><source type="video/mp4"></video>

![](https://pbs.twimg.com/amplify_video_thumb/2082305193253728256/img/OkI0GkeDkXDwZlOb.jpg?name=large)
...
> The reason is structural rather than a matter of intelligence. The context where the code was written is packed with every argument for writing it that way, so on reread the agent sees its own reasoning instead of the result. Writers hit the identical failure - you reread a draft and see the version in your head.
...
![画像](https://pbs.twimg.com/media/HOXWfSQW8AAT93R?format=jpg&name=large)
...
2. **Write that run down as a skill.** Conventions, build steps, the workaround your team keeps for that one flaky runner. Have the schedule fire a named skill rather than a wall of pasted instructions, because instructions inside a cron job never get updated by anyone, ever.
3. **Add a state file.** STATE.md at the repo root. Version controlled, diff readable, boring.
4. **Put a gate and a cap around it.** An objective check plus a turn limit. This is the step where the loop gains the ability to fail.
5. **Then schedule it.** /loop while you are still watching. /schedule once you trust it unattended.
6. **Bring in a verifier subagent.** A second agent in .claude/agents/, its own instructions, ideally a different model.
...
![画像](https://pbs.twimg.com/media/HObt03YWEAENB9M?format=jpg&name=large)
...
![画像](https://pbs.twimg.com/media/HOXXt9hWsAA_j9-?format=jpg&name=large)

## 8/ Two Loops Already Running

> **One person, one morning.** Osmani's own triage loop wakes up before he does. A triage skill reads yesterday's CI failures, open issues and recent commits. Every finding worth acting on opens its own isolated worktree. One sub-agent drafts the fix, a second attacks it against the project's tests and skills. Connectors open the PR and update the ticket, anything ambiguous lands in an inbox for a human, and the state file carries leftovers into tomorrow.
...
2. **Reliability came from constraints, not model size.** Minions is a fork of the open-source tool Goose, not a secret frontier model.
3. **The humans did not leave.** Those 1,300 PRs are still reviewed by engineers. They changed desks, from writing to reviewing.

![画像](https://pbs.twimg.com/media/HOXgctfXgAApudP?format=jpg&name=large)

> ⚠️ **On the numbers flying around.** "90 percent of Claude Code writes itself" and similar headline figures are mostly secondhand. Anthropic's own 8x merge-rate figure comes with the company calling it almost certainly an overstatement of the real gain. The two cases above trace to firsthand sources, which is why they are the ones worth reasoning from.
...
**Verification debt** is where the Ralph Wiggum loop lives, named by Geoffrey Huntley: the agent fires the completion signal on a half-finished job and the run exits pleased with itself. The cure is a gate with no opinion, something that compiles or fails, returns zero or non-zero. A reviewer with an opinion eventually argues itself into approval.
...
> 7月27日
> 
> How to run your whole workday on loops instead of prompts loop-engineering ┃ ┣ Contract ┃ ┣ /claude-md ┃ ┣ /house-rules ┃ ┣ /tone-and-scope ┃ ┗ /stop-conditions ┃ ┣ Loop Types ┃ ┣ /turn-based ┃ ┣ /goal-based ┃ ┣ /time-based ┃ ┣
...
![画像](https://pbs.twimg.com/media/HOXgOfIWEAAKFTI?format=jpg&name=large)

## 11/ What This Is Worth in Money

- **Contract setup work.** Small teams pay [$3k-8k](https://x.com/search?q=%243k-8k&src=cashtag_click) to have someone stand up CI triage, dependency automation and an overnight review loop. Everyone internally wants it and nobody has a free week to build it.
...
![画像](https://pbs.twimg.com/media/HOaOgQbXoAA2XJI?format=jpg&name=large)
...
![画像](https://pbs.twimg.com/media/HOXPYo-WcAAZam9?format=jpg&name=large)

**The stack:**

📁 Claude Code loops guide ↳ [claude.com/docs/en/scheduled-tasks](https://code.claude.com/docs/en/scheduled-tasks)

📁 /goal documentation ↳ [claude.com/docs/en/goal](https://code.claude.com/docs/en/goal)
