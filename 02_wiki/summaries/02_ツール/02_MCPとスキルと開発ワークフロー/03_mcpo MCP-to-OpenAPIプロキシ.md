---
title: "03_mcpo MCP-to-OpenAPIプロキシ"
...
- **stdio・SSE・Streamable HTTP のトランスポート対応** は、[[04_ephemeral-sandbox 並列エージェント用OSSサンドボックス基盤]]（MCP インターフェース・3ツール群）と同様に「エージェントとツールを繋ぐ層」の標準化
...
- MCP インターフェース（エージェントとツールの接続層） → [[04_ephemeral-sandbox 並列エージェント用OSSサンドボックス基盤]]
