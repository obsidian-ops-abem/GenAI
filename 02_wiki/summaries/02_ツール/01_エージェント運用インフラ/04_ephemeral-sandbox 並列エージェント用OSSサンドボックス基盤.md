---
title: "04_ephemeral-sandbox 並列エージェント用OSSサンドボックス基盤"
...
エージェント並列実行の**インフラ（隔離）層**。[[01_Vibe Kanban コーディングエージェント用Kanbanワークスペース]]（エージェント並列の**人間側UI**・差分レビュー）や [[02_agmsg CLIエージェント間メッセージング]]（エージェント間の**通信**）が「エージェント同士／人間とエージェント」のインターフェースなら、ephemeral-sandbox は**エージェントが安全に並列で書き込む土俵**を提供する。

特に「ワークスペース隔離」の概念は [[02_1チャットをエージェントチームへ Opus5 12ステップ]]（worktree 隔離）や [[03_Graph of Loops Claude Code完全システム10リポジトリ]]（G2 agent-worktree）と同系統だが、本作は**製品としてのインフラ基盤**（gateway/daemon/observability を含むヘッドレスコア）を提供する点が異なる。「atomic publication（全部解けるか、さもなくば無）」は [[02_24時間自走する自律型AIエージェントの設計図]]（Guardrail 層・安全な停止）の思想と通底。

## 関連

- エージェント並列のワークスペース分離 → [[02_1チャットをエージェントチームへ Opus5 12ステップ]]・[[03_Graph of Loops Claude Code完全システム10リポジトリ]]
- エージェント並列の周辺（UI・通信） → [[01_Vibe Kanban コーディングエージェント用Kanbanワークスペース]]・[[02_agmsg CLIエージェント間メッセージング]]
