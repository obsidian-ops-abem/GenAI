...
## 続報：v0.9.1 で発覚した「グローバル名前空間の罠」（2026-08-03）

v0.9.0 リリース後、別チームが2チーム目を立てようとして詰まり、v0.9.1 で修正（出典: [[78_intent-cli v0.9.1 グローバル名前空間の罠と修正（出典）]]）。

**問題**：herdr のエージェント名はマシン全体でグローバル一意。一方 intent-cli の通知は「ロール名と同名のエージェント」を探していた。`implementation` や `review` という正規名はマシン上の1チームしか持てず、**2チーム目の通知が必ず壊れる構造**だった。

**気づきの決め手**：このマシンの4ワークスペースが全部違う命名を発明していた（`implementation` / `implement` / `sekiban-implementation`）。誰も同じ名前を使えていないこと自体が、グローバル名前空間の衝突の署名だった。

**修正**：
1. **受信者の同一性を「記録された workspace + pane」に変更** — ロール名がエージェント名から独立し、複数チームがそれぞれ正規名を使用可能に
2. **記録無しを「検査不要」でなく「検査未了」として扱う** — 設定未記録のチームがこれまで健全性チェックを素通りしていたのを修正

agmsg 版も引き続き利用可能。

→ 本記事の「複数ロール・複数ドメインを衝突させない工夫」が**実運用の2チーム目で真価を問われた**事例。グローバル名前空間の暗黙前提が、スケール時に署名として現れる典型。[[05_マージゲート4シグナル 信頼スコアの罠とレーン分離]] の「最も弱い変数に最大加重」の別形：ここでは「名前の一意性」という暗黙前提がスケールで破綻。
