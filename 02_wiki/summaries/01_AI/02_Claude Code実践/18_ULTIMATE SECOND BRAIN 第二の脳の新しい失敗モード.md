---
title: "18_ULTIMATE SECOND BRAIN 第二の脳の新しい失敗モード"
...
- **Karpathy gist 直接参照** → [[12_カーパシーのObsidian活用術 30分で第二の脳]]（Karpathy LLM Wiki 設計図）・[[01_Claude×Obsidianで第二の脳を作る]]（概念版）・[[09_Claude Code×Obsidianで第二の脳を作る 2000時間の設計ルール]]（熟練者版）と同系譜の統合版
- **1M window が retrieval pipeline を不要化** → [[07_Everything Fable 5 Mythosクラスとプロンプトガイド]]（1M コンテキスト・長期曖昧問題で真価）・[[04_知識グラフメモリをOpus5で安く運用する]]（キャッシュ/バッチでコスト下げる）と直結。本ボルト規模（中型）なら1リクエスト収まりの可能性
- **「2000ノート超で劣化」** は本ボルトが `02_wiki/summaries/` を raw sprawl でなく compiled page に保つ根拠。pruning beats summarizing（[[16_Claude Codeの6層アーキテクチャ ダムループ]]）と整合
- **「conservative な決定」**（保守をエージェントへ）は [[15_CLINEに全部賭けろ コーディングエージェント時代のプログラマ]]（ドライバー席を譲る）・[[17_Stop Vibe Coding Spec駆動開発の5ブロック]]（タイプする人→決定する人）と同じ人間役割の転換

---

## 関連

- Karpathy LLM Wiki（原型） → [[12_カーパシーのObsidian活用術 30分で第二の脳]]
- 概念版（3層・Ingest/Query/Lint） → [[01_Claude×Obsidianで第二の脳を作る]]
- 熟練者版（2000時間運用） → [[09_Claude Code×Obsidianで第二の脳を作る 2000時間の設計ルール]]
- 1M コンテキストで retrieval 不要化 → [[07_Everything Fable 5 Mythosクラスとプロンプトガイド]]
- Opus5 で知識グラフメモリを安く → [[04_知識グラフメモリをOpus5で安く運用する]]
- pruning beats summarizing → [[16_Claude Codeの6層アーキテクチャ ダムループ]]
- 人間は決定する人へ → [[17_Stop Vibe Coding Spec駆動開発の5ブロック]]
