---
title: "03_Graph of Loops Claude Code完全システム10リポジトリ"
...
| 既存ノートの軸 | 本ノートの位置 |
|---|---|
| [[04_Agent Harness vs Loop vs Graph Engineering]]・[[01_LOOP vs GRAPH vs HARNESS ENGINEERING]] 3層並列比較 | **Graph+Loop の実装統合**（Harness は G1/L3 等） |
| [[02_Loop Engineering Claude,GPT 実戦で効くもの]] Loop↔Chain | **Graph 層を Loop の上にどう構築するか** |
| [[02_1チャットをエージェントチームへ Opus5 12ステップ]] サブエージェント | G2 worktree・G3 役割分割の具体実装 |
| [[02_Graph Engineering 最大の間違い Loop↔Graph判断]] Loop↔Graph判断 | 「まず Loop（L1-L6）で信頼できるノードを作ってから Graph（G1-G4）」と整合 |
| [[01_エージェントファクトリの作り方 ビルダーズガイド]] ファクトリ | 「done を取り戻せるか」= no evals no production / ゲート思想と同一 |

特に「**done を取り戻せるか**」は [[01_エージェントファクトリの作り方 ビルダーズガイド]] の「no evals, no production」や [[02_Loop Engineering Claude,GPT 実戦で効くもの]]「接地した検証器」と同じ根拠ベース停止の原則。10リポジトリの実装例は、抽象論を実コードへ落とす参照として非常に実用的。

## 関連

- Graph+Loop 二層 → [[04_Agent Harness vs Loop vs Graph Engineering]]・[[01_LOOP vs GRAPH vs HARNESS ENGINEERING]]
- Loop 信頼性・ゲート → [[02_Loop Engineering Claude,GPT 実戦で効くもの]]・[[01_エージェントファクトリの作り方 ビルダーズガイド]]
- worktree・ファンアウト → [[02_1チャットをエージェントチームへ Opus5 12ステップ]]・[[04_ephemeral-sandbox 並列エージェント用OSSサンドボックス基盤]]
- 「done を取り戻せる」= 根拠停止 → [[02_24時間自走する自律型AIエージェントの設計図]]
- Graph は必要時だけ → [[02_Graph Engineering 最大の間違い Loop↔Graph判断]]・[[02_Graph Engineering Clearly Explained]]
