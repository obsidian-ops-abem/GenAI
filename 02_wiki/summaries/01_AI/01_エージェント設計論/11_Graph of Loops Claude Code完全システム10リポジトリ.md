---
title: "11_Graph of Loops Claude Code完全システム10リポジトリ"
...
| 既存ノートの軸 | 本ノートの位置 |
|---|---|
| [[08_Agent Harness vs Loop vs Graph Engineering]]・[[02_LOOP vs GRAPH vs HARNESS ENGINEERING]] 3層並列比較 | **Graph+Loop の実装統合**（Harness は G1/L3 等） |
| [[09_Loop Engineering Claude,GPT 実戦で効くもの]] Loop↔Chain | **Graph 層を Loop の上にどう構築するか** |
| [[10_1チャットをエージェントチームへ Opus5 12ステップ]] サブエージェント | G2 worktree・G3 役割分割の具体実装 |
| [[10_Graph Engineering 最大の間違い Loop↔Graph判断]] Loop↔Graph判断 | 「まず Loop（L1-L6）で信頼できるノードを作ってから Graph（G1-G4）」と整合 |
| [[01_エージェントファクトリの作り方 ビルダーズガイド]] ファクトリ | 「done を取り戻せるか」= no evals no production / ゲート思想と同一 |

特に「**done を取り戻せるか**」は [[01_エージェントファクトリの作り方 ビルダーズガイド]] の「no evals, no production」や [[09_Loop Engineering Claude,GPT 実戦で効くもの]]「接地した検証器」と同じ根拠ベース停止の原則。10リポジトリの実装例は、抽象論を実コードへ落とす参照として非常に実用的。
