---
title: "04_Agent Harness vs Loop vs Graph Engineering"
...
Graph が価値を持つのは: 意味のある分岐 / 承認 / スペシャリストの引継ぎ / 並列 / リカバリパス / 明示的な制御点を持つ複数ステップワークフロー。「1人のエージェントにツールを渡して任せる」だけなら harness + loop で十分。**早すぎるグラフはシステムを脆くする**（→ [[02_Graph Engineering 最大の間違い Loop↔Graph判断]] と同系）。
...
2. **同じモデルに書かせて採点させる** — 同じ盲点を共有。決定的チェック / 別レビューア文脈 / 外部評価器 / 高影響行動への人間承認を（→ [[04_自己レビューエージェントのGraph設計 Anthropicメソッド]] と同系）
...
- **Loop から始める**（基本）→ [[01_ループエンジニアリング14ステップ]]
- **Graph へ**: 非線形・分岐・並列、複数特化ステップ間の状態共有、単一ループの記憶過多/待ち状態 → [[02_Graph Engineering Clearly Explained]]
...
- [[01_LOOP vs GRAPH vs HARNESS ENGINEERING]] — 同3層の診断フレーム
- [[02_Prompt to Graph Engineering 5層の統一モデル]] — Prompt/Context/**Harness**/Loop/Graph の5層（前編）
- [[02_Graph Engineering Clearly Explained]] — Graph 層の実践解説（続編）
- [[01_LOOP→GRAPH→HARNESSパイプラインを一気通貫で実装する]] — 3層の実装チュートリアル
...
- 3層診断フレーム → [[01_LOOP vs GRAPH vs HARNESS ENGINEERING]]
- 5層統一モデル（前編） → [[02_Prompt to Graph Engineering 5層の統一モデル]]
- Graph 層の深掘り（続編） → [[02_Graph Engineering Clearly Explained]]
- 3層の実装 → [[01_LOOP→GRAPH→HARNESSパイプラインを一気通貫で実装する]]
- Loop 層のロードマップ → [[01_ループエンジニアリング14ステップ]]
